...
(function($, undefined) {
	...
	function Colio(list, settings){
		...
		// adjust colio viewport on window resize
	...
	Colio.prototype.expandViewport = function(content_id, adjust){
		...
		this.markup.stop().animate({height: viewport_height}, duration, easing, $.proxy(function(){
			...
			if(typeof this.settings.onExpand === 'function') {
				this.settings.onExpand.call(this.markup.get(0), this.items.eq(this.position).get(0));
			}
	...
	Colio.prototype.collapse = function(){
		...
		this.markup.stop().animate({height:0}, duration, easing, $.proxy(function(){
			...
			if(typeof this.settings.onCollapse === 'function') {
				this.settings.onCollapse.call(this.markup.get(0), this.items.eq(this.position).get(0));
			}
	...
	*/
	...
	$.fn.colio = function(c) {
		// default settings
		var s = $.extend({
			id: 'colio',						// ID to be assigned to colio viewport
			...
			onContent: function(){}				// on content load callback
		}, c);
